...
//  DistributorRole private _distributorRole;
//  RetailerRole private _retailerRole;
//  Ownable private _ownalbe;
  ...
  modifier checkValue(uint _upc) {
    ...
    if(amountToReturn > 0) {
      address  payAddress = 0xd1F010473786848Bf3A5fb70D2c4D7D81Cfdd842;
        address payable addressPayable = _make_payable(payAddress);
        addressPayable.transfer(1);
...
//      selfdestruct(owner);
  ...
  function addRoles(address farmerAddress, address distributorAddress, address retailerAddress, address consumerAddress ) public {
//    addFarmer(farmerAddress);
//    addDistributor(distributorAddress);
//    addRetailer(retailerAddress);
//    addConsumer(consumerAddress);
  }
  ...
  function harvestItem(uint _upc, address _originFarmerID, string memory _originFarmName, string memory _originFarmInformation, string memory _originFarmLatitude, string  memory _originFarmLongitude, string memory _productNotes) public
  {
    ...
    Item memory item = Item(sku, _upc, msg.sender, _originFarmerID, _originFarmName, _originFarmInformation,
      _originFarmLatitude, _originFarmLongitude,_upc+sku,_productNotes, 0, State.Harvested ,address(0),address(0),address(0));
    // Increment sku
    items[_upc] = item;
    sku = sku + 1;
  ...
  function buyItem(uint _upc)
    ...
    checkValue(msg.value) public payable
    {
...
//      address  payAddress = 0xd1F010473786848Bf3A5fb70D2c4D7D81Cfdd842;
//      secondAddress.transfer(1);
      address payable addressPayable = _make_payable(items[_upc].originFarmerID);
  ...
  function receiveItem(uint _upc)
    ...
    {
      require(isRetailer(msg.sender), "You don't have retailer role");
  ...
  function purchaseItem(uint _upc)
    // Call modifier to check if upc has passed previous supply chain stage
    received(_upc) public
    // Access Control List enforced by calling Smart Contract / DApp
    {
      require(isConsumer(msg.sender), "You don't have consumer role");
  ...
  ) 
  {
    ...
    productPrice = uint2str(item.productPrice);
